...
void print_event(const struct parser_event *event)
{
    if (event == NULL)
        return;
    printf("EVENT TYPE: %s; EVENT CHARS: ", parser_utils_strcmpi_event(event->type));
    for (int i = 0; i < event->n; i += 1)
    {
        putchar(event->data[i]);
    }
    putchar('\n');
    print_event(event->next);
}